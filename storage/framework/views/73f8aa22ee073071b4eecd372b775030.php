        ...
        <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $step): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden">
                <div class="p-6 text-center">
                    ...
                    <div class="text-gray-400 text-lg mb-2"><i class="<?php echo e($step['icon'] ?? 'fa-solid fa-circle'); ?>"></i></div>
                    <h6 class="font-bold text-gray-900 mb-1"><?php echo e($step['title']); ?></h6>
                    <p class="text-gray-500 text-sm leading-relaxed"><?php echo e(Str::limit($step['description'], 80)); ?></p>
                    <span class="inline-block mt-3 text-xs font-semibold px-3 py-1 rounded-full <?php echo e($step['is_active'] ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-gray-50 text-gray-400 border border-gray-200'); ?>">
                        <?php echo e($step['is_active'] ? 'Activo' : 'Inactivo'); ?>

                    </span>
                </div>
                <div class="border-t border-gray-100 bg-gray-50/80 px-4 py-3.5 flex justify-center gap-2">
                    <button class="w-9 h-9 flex items-center justify-center rounded-lg bg-amber-50 border border-amber-200 text-amber-600 hover:bg-amber-100 transition-all" wire:click="edit(<?php echo e($step['id']); ?>)">
                        <i class="fa-solid fa-pen text-xs"></i>
                    </button>
                    <button class="w-9 h-9 flex items-center justify-center rounded-lg border transition-all <?php echo e($step['is_active'] ? 'bg-gray-50 border-gray-200 text-gray-400 hover:bg-gray-100' : 'bg-green-50 border-green-200 text-green-600 hover:bg-green-100'); ?>" wire:click="toggleActive(<?php echo e($step['id']); ?>)">
                        <i class="fa-solid fa-<?php echo e($step['is_active'] ? 'eye-slash' : 'eye'); ?> text-xs"></i>
                    </button>
                    <button class="w-9 h-9 flex items-center justify-center rounded-lg bg-red-50 border border-red-200 text-red-500 hover:bg-red-100 transition-all" wire:click="delete(<?php echo e($step['id']); ?>)" wire:confirm="¿Eliminar este paso?">
        ...
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-start justify-center z-50 overflow-y-auto py-8 px-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-auto">
                <div class="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center rounded-t-2xl z-10">
                    <h5 class="text-lg font-bold text-gray-900"><?php echo e($editingId ? 'Editar' : 'Nuevo'); ?> Paso</h5>
...
<?php /**PATH C:\xampp\htdocs\arturo-motors\resources\views/livewire/cms/gestionar-pasos.blade.php ENDPATH**/ ?>