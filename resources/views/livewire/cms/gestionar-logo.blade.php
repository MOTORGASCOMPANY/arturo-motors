<x-cms.layout
    ...
    description="Sube y gestiona el logo principal y el favicon del sitio"
    headerIcon='<i class="fa-solid fa-image text-blue-600"></i>'
>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-8 border border-gray-200 dark:border-gray-700 min-h-[200px] flex items-center justify-center">
                        @if($currentLogo)
                            <div class="relative max-w-full max-h-[180px]">
                                <picture>
                                    @if($currentLogo->webpUrl())
                                        <source type="image/webp" srcset="{{ $currentLogo->webpUrl() }}">
                                    @endif
                                    <img src="{{ asset('storage/' . $currentLogo->file_path) }}" alt="Logo actual" class="max-w-full max-h-[180px] object-contain">
                                </picture>
                                <div class="absolute top-2 right-2">
<button type="button" onclick="confirmRemoveLogo()" class="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center shadow-lg hover:bg-red-600 transition-all" aria-label="Eliminar logo">
                ...
                <div class="md:col-span-1 space-y-5">
                    ...
                    @if($currentLogo)
                        <button type="button" class="w-full px-5 py-2.5 rounded-xl bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 font-semibold transition-all flex items-center justify-center gap-2" onclick="confirmRemoveLogo()">
    ...
    {{-- Favicon Section --}}
    <x-cms.card class="mb-8" style="animation: cardEntry 0.4s ease-out 0.1s both">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-8 border border-gray-200 dark:border-gray-700 min-h-[200px] flex flex-col items-center justify-center">
                        @if($currentFavicon)
                            <div class="relative mb-4">
                                ...
                                <div class="absolute top-2 right-2">
<button type="button" onclick="confirmRemoveFavicon()" class="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center shadow-lg hover:bg-red-600 transition-all" aria-label="Eliminar favicon">
                                         <i class="fa-solid fa-trash text-sm"></i>
                                     </button>
                                </div>
                            </div>
                ...
                <div class="md:col-span-1 space-y-5">
                    ...
                    @if($currentFavicon)
                        <button type="button" class="w-full px-5 py-2.5 rounded-xl bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 font-semibold transition-all flex items-center justify-center gap-2" onclick="confirmRemoveFavicon()">
    ...
    {{-- Preview in Navbar --}}
    <x-cms.card style="animation: cardEntry 0.4s ease-out 0.2s both">
        <div class="p-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                <nav class="flex items-center justify-between" style="background: rgba(255,255,255,0.95); backdrop-filter: blur(12px);">
                    <a href="#" class="flex items-center gap-2" style="color: #0a0f1e;">
                        @if($currentLogo)
                            <img src="{{ $logoPreview }}" alt="Logo" class="h-8 w-auto" style="max-width: 120px;">
                        @else
    ...
    <script>
        function confirmRemoveLogo() {
            Swal.fire({
                title: '¿Eliminar logo?',
                text: 'Esta acción eliminará el logo actual permanentemente.',
                icon: 'warning',
        ...
        function confirmRemoveFavicon() {
            Swal.fire({
                ...
                text: 'Esta acción eliminará el favicon actual permanentemente.',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#dc2626',
                cancelButtonColor: '#64748b',
