<div>
    <style>
        ...
        @keyframes cardExpand { from { opacity: 0; transform: perspective(600px) rotateX(8deg); } to { opacity: 1; transform: perspective(600px) rotateX(0deg); } }
        ...
        @forelse ($cards as $card)
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden" style="animation: cardEntry 0.4s ease-out {{ $loop->index * 0.06 }}s both">
                <div class="p-6 text-center">
                    <div class="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 text-2xl mx-auto mb-4 border border-blue-100 hover:bg-blue-100 hover:border-blue-200 transition-all cursor-pointer">
                        <i class="{{ $card['icon'] ?? 'fa-solid fa-star' }}"></i>
                    </div>
                    <h6 class="font-bold text-gray-900 mb-1">{{ $card['title'] }}</h6>
                    <p class="text-gray-500 text-sm leading-relaxed">{{ Str::limit($card['description'], 80) }}</p>
                    <span class="inline-block mt-3 text-xs font-semibold px-3 py-1.5 rounded-full {{ $card['is_active'] ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-gray-50 text-gray-400 border border-gray-200' }}">
                        {{ $card['is_active'] ? 'Activa' : 'Inactiva' }}
                    </span>
                </div>
                <div class="border-t border-gray-100 bg-gray-50/80 px-4 py-3.5 flex justify-center gap-2">
                    <button class="w-9 h-9 flex items-center justify-center rounded-lg bg-amber-50 border border-amber-200 text-amber-600 hover:bg-amber-100 transition-all" wire:click="edit({{ $card['id'] }})">
                        <i class="fa-solid fa-pen text-xs"></i>
                    </button>
                    <button class="w-9 h-9 flex items-center justify-center rounded-lg border transition-all {{ $card['is_active'] ? 'bg-gray-50 border-gray-200 text-gray-400 hover:bg-gray-100' : 'bg-green-50 border-green-200 text-green-600 hover:bg-green-100' }}" wire:click="toggleActive({{ $card['id'] }})">
                        <i class="fa-solid fa-{{ $card['is_active'] ? 'eye-slash' : 'eye' }} text-xs"></i>
                    </button>
                    <button class="w-9 h-9 flex items-center justify-center rounded-lg bg-red-50 border border-red-200 text-red-500 hover:bg-red-100 transition-all"
                            onclick="window.dispatchEvent(new CustomEvent('confirm-modal:show', { detail: { title: 'Eliminar razón', message: '¿Seguro que querés eliminar esta razón? Esta acción no se puede deshacer.', action: { componentId: $wire.__instance.id, method: 'delete', params: [{{ $card['id'] }}] } } }))">
    ...
    @if($showForm)
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-start justify-center z-50 overflow-y-auto py-8 px-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-auto" style="animation: cardExpand 0.4s ease-out">
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <div class="w-10 h-10 rounded-xl bg-blue-500 flex items-center justify-center text-white text-lg shadow-md shadow-blue-500/25">
                            <i class="{{ $icon ?: 'fa-solid fa-star' }}"></i>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-700">Vista previa del acento</p>
                            <p class="text-xs text-gray-400">Así se verá en la landing</p>
                        </div>
                    </div>
                    <div>
