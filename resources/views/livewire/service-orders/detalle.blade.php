            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-700 flex items-center gap-1 mt-0.5">
                        ...
                        <span class="font-medium text-gray-800">{{ $orden->creadoPor->name }}</span>
                    </p>
                </div>
            </div>
            <span
    ...
    <div class="bg-gray-200 rounded-xl shadow-sm border border-gray-300/80 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <div>
                ...
                <p class="font-bold text-gray-900">{{ $orden->vehiculo->placa }} — {{ $orden->vehiculo->marca }}
                    {{ $orden->vehiculo->modelo }}</p>
    ...
    <div class="bg-gray-200 rounded-xl shadow-sm border border-gray-300/80 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            ...
            <div>
                ...
                <p class="font-bold {{ $orden->precio_final != $orden->precio_lista ? 'text-amber-700' : 'text-gray-900' }}">
    ...
    @if ($orden->service->tipo === 'conversion' && $orden->checklist_evaluacion)
        <div class="bg-gray-200 rounded-xl shadow-sm border border-gray-300/80 p-6">
            ...
            <div class="flex items-center gap-3 mb-4 text-sm">
                <span class="px-2.5 py-1 rounded-full text-xs font-bold {{ $orden->evaluacion_aprobada ? 'bg-emerald-200 text-emerald-900' : 'bg-red-200 text-red-900' }}">
                    {{ $orden->evaluacion_aprobada ? 'Apto' : 'No apto' }}
                </span>
                <span class="text-gray-700 font-medium">
                    Por {{ $orden->evaluadoPor?->name }} — {{ $orden->evaluado_en?->format('d/m/Y H:i') }}
                </span>
            </div>
                ...
                @foreach ($checklistGrupos as $items)
                    @foreach ($items as $clave => $label)
                        <div class="flex items-center gap-1.5 py-1">
                            @if ($orden->checklist_evaluacion[$clave] ?? false)
        ...
        <div class="bg-gray-200 rounded-xl shadow-sm border border-gray-300/80 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm mb-4">
                ...
                <div>
                    ...
                    <p class="font-bold text-gray-900">{{ $orden->fecha_inicio_conversion?->format('d/m/Y H:i') ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-gray-600 text-xs font-medium">Fin</p>
                    <p class="font-bold text-gray-900">{{ $orden->fecha_fin_conversion?->format('d/m/Y H:i') ?? '—' }}</p>
                </div>
            </div>
                    ...
                    @foreach ($orden->items as $item)
                        <div class="flex justify-between text-sm bg-white/80 border border-gray-300 rounded-lg px-3 py-2">
                            <span class="font-medium text-gray-800">
                                {{ $item->producto->categoria->nombre }} — {{ $item->producto->nombre }} ({{ $item->producto->marca }})
                            </span>
        ...
        @if ($orden->comprobante)
            <div class="flex items-center justify-between text-sm">
                <div>
                    ...
                    <p class="text-gray-700 text-xs font-medium">{{ ucfirst($orden->comprobante->metodo_pago) }} —
    ...
    @if ($orden->service->tipo === 'conversion')
        <div class="bg-gray-200 rounded-xl shadow-sm border border-gray-300/80 p-6">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @endif
                @if (in_array($orden->estado, ['entregado', 'entregada']))
                    ...
                    <div>
                        <p class="text-gray-900">
                            @if ($h->estado_anterior)
                                <span class="text-gray-600">{{ ucfirst(str_replace('_', ' ', $h->estado_anterior)) }}</span> →
                            @endif
                            <span class="font-bold text-gray-900">{{ ucfirst(str_replace('_', ' ', $h->estado_nuevo)) }}</span>
                        </p>
