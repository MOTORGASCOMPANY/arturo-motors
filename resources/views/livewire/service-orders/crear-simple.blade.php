<div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow">
    ...
    <div class="flex justify-between mb-6 text-sm font-medium">
        <span class="{{ $paso >= 1 ? 'text-indigo-600' : 'text-gray-400' }}">1. Cliente y vehículo</span>
        <span class="{{ $paso >= 2 ? 'text-indigo-600' : 'text-gray-400' }}">2. Servicio</span>
        <span class="{{ $paso >= 3 ? 'text-indigo-600' : 'text-gray-400' }}">3. Cobro</span>
        <span class="{{ $paso >= 4 ? 'text-indigo-600' : 'text-gray-400' }}">4. Listo</span>
    </div>
    ...
    @if ($paso === 1)
        <div class="space-y-4">
            <div>
                ...
                @endif
                @error('clienteId') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
            </div>
            ...
            @if ($clienteId)
                <div>
                    ...
                    @error('vehiculoId') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 gap-2">
                @foreach ($servicios as $s)
                    <label class="flex items-center justify-between border rounded-md p-3 cursor-pointer
                                   {{ $serviceId === $s->id ? 'border-indigo-600 bg-indigo-50' : '' }}">
                        <span>
                            <input type="radio" wire:click="seleccionarServicio({{ $s->id }})" class="mr-2">
                            {{ $s->nombre }}
                        </span>
                        <span class="text-gray-500">S/ {{ number_format($s->precio_base, 2) }}</span>
                    </label>
                @endforeach
            </div>
            @error('serviceId') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
            ...
            @if ($serviceId)
                <div>
                    ...
                    <p class="text-xs text-gray-500">Precio de lista: S/ {{ number_format($precioLista, 2) }}</p>
                </div>
                ...
                <div>
                    ...
                    @error('descuentoMotivo') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                </div>
            @endif
    ...
    @if ($paso === 3)
        <div class="space-y-4">
            <p class="text-lg">Total a cobrar: <strong>S/ {{ number_format($precioFinal, 2) }}</strong></p>
            ...
            @error('caja') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
    ...
    @if ($paso === 4)
        <div class="text-center space-y-3">
            <p class="text-green-600 text-xl">✔ Orden #{{ $ordenCreadaId }} registrada</p>
