            ...
            @else
                <div x-data="{
                    ...
                    async handleSubmit(event) {
                        ...
                        // Get current montoCierre from Livewire
                        const montoCierre = parseFloat($wire.montoCierre) || 0;
                        const montoEsperado = {{ $this->montoEsperado }};
                        const diff = Math.abs(montoCierre - montoEsperado);
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative rounded-xl shadow-xs">
                                ...
                                <x-input id="montoCierre" type="text" inputmode="decimal" autocomplete="off"
                                         wire:model.live="montoCierre"
                        ...
                        <div class="pt-2">
                            <button type="submit"
                                    :disabled="submitting"
                                    class="w-full justify-center bg-red-600 hover:bg-red-700 active:bg-red-800 text-white font-medium py-3 rounded-xl shadow-xs transition-all text-sm inline-flex items-center disabled:opacity-50">
                                <template x-if="!submitting">
                                    <span><i class="fas fa-lock mr-2"></i>Cerrar Caja Ahora</span>
                                </template>
                                <template x-if="submitting">
                                    <span><i class="fas fa-spinner fa-spin mr-2"></i>Procesando Cierre...</span>
                                </template>
                            </button>
                        </div>
                    </form>
