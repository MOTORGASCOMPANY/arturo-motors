    ...
    <div class="bg-gradient-to-r from-slate-900 via-indigo-900 to-blue-900 rounded-2xl shadow-lg p-5 sm:p-6 text-white flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        @role('Vendedor|Jefe de Taller|Administrador del sistema')
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('conversiones.crear') }}" 
        ...
        <a href="{{ $sesionCaja ? route('caja.historial') : route('caja.abrir') }}"
           class="md:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4 hover:shadow-md transition-all duration-150 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                ...
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold {{ $sesionCaja ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">
                    <span class="w-1.5 h-1.5 rounded-full {{ $sesionCaja ? 'bg-emerald-600 animate-pulse' : 'bg-rose-600' }}"></span>
                ...
                @if ($sesionCaja)
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-emerald-50/60 rounded-xl p-2.5 border border-emerald-100/60">
                            ...
                            <span class="text-base font-extrabold text-emerald-700">S/ {{ number_format($ingresosHoy, 2) }}</span>
                        </div>
                        <div class="bg-rose-50/60 rounded-xl p-2.5 border border-rose-100/60">
                            ...
                            <span class="text-base font-extrabold text-rose-700">S/ {{ number_format($egresosHoy, 2) }}</span>
                        </div>
                    </div>
                    <p class="text-[11px] text-gray-400 mt-2 flex items-center gap-1">
                        <i class="fas fa-user-circle"></i> Abierta por {{ $sesionCaja->abiertaPor->name }}
                    </p>
                @else
        ...
        <a href="{{ route('ordenes.listado') }}"
           class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4 hover:shadow-md transition-all duration-150 flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-black text-gray-800 mt-1">{{ $ordenesHoy }}</p>
            ...
            <a href="{{ route('conversiones.asignar') }}"
               class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4 hover:shadow-md transition-all duration-150 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-black {{ $pendientesAsignarTecnico > 0 ? 'text-amber-600' : 'text-gray-800' }} mt-1">
                        {{ $pendientesAsignarTecnico }}
                    </p>
                    <span class="text-[11px] text-gray-400 font-medium">Técnico pendiente</span>
                </div>
                <div class="w-11 h-11 rounded-2xl {{ $pendientesAsignarTecnico > 0 ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-gray-500' }} flex items-center justify-center shrink-0">
            ...
            <a href="{{ route('conversiones.entregas-pendientes') }}"
               class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4 hover:shadow-md transition-all duration-150 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-black {{ $pendientesEntrega > 0 ? 'text-amber-600' : 'text-gray-800' }} mt-1">
                        {{ $pendientesEntrega }}
                    </p>
                    <span class="text-[11px] text-gray-400 font-medium">Vehículos listos</span>
                </div>
                <div class="w-11 h-11 rounded-2xl {{ $pendientesEntrega > 0 ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-gray-500' }} flex items-center justify-center shrink-0">
            ...
            <a href="{{ route('conversiones.mis-asignadas') }}"
               class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4 hover:shadow-md transition-all duration-150 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-black {{ $misPendientes > 0 ? 'text-amber-600' : 'text-gray-800' }} mt-1">
                        {{ $misPendientes }}
                    </p>
                    <span class="text-[11px] text-gray-400 font-medium">Asignadas a mí</span>
                </div>
                <div class="w-11 h-11 rounded-2xl {{ $misPendientes > 0 ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-gray-500' }} flex items-center justify-center shrink-0">
            ...
            <a href="{{ route('conversiones.almacen-pendientes') }}"
               class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4 hover:shadow-md transition-all duration-150 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-black {{ $pendientesAlmacen > 0 ? 'text-amber-600' : 'text-gray-800' }} mt-1">
                        {{ $pendientesAlmacen }}
                    </p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-4">
        ...
        <div class="grid grid-cols-3 gap-3 divide-x divide-gray-100 text-center">
            <div class="px-2">
                ...
                <p class="text-xl sm:text-2xl font-black text-purple-700 mt-0.5">{{ $conversionesHoy }}</p>
            </div>
            <div class="px-2">
                <span class="text-[11px] font-semibold text-gray-400 block uppercase">Esta semana</span>
                <p class="text-xl sm:text-2xl font-black text-purple-700 mt-0.5">{{ $conversionesSemana }}</p>
            </div>
            <div class="px-2">
                <span class="text-[11px] font-semibold text-gray-400 block uppercase">Este mes</span>
                <p class="text-xl sm:text-2xl font-black text-purple-700 mt-0.5">{{ $conversionesMes }}</p>
            </div>
        </div>
    </div>
