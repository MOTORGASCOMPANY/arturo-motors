<div wire:loading.class="opacity-50 pointer-events-none" class="max-w-2xl mx-auto px-4 py-8">
    <div class="bg-gray-200 rounded-2xl shadow-md border border-gray-300 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-300/80 bg-gray-200">
            <h2 class="text-xl font-bold text-gray-900">Realizar conversión — Orden #{{ $orden->id }}</h2>
            <p class="text-sm text-gray-600 mt-1">
                {{ $orden->cliente->nombre }} {{ $orden->cliente->apellido }} —
                <span class="font-bold text-gray-800">{{ $orden->vehiculo->placa }}</span> 
                <span class="text-gray-500">({{ $orden->service->nombre }})</span>
            </p>
        </div>
                    ...
                    @forelse ($orden->items as $item)
                        <div class="flex justify-between items-center border border-gray-300/90 rounded-xl px-3.5 py-2.5 text-sm bg-gray-100 shadow-sm">
                            <span class="text-gray-800">
                                <strong class="font-semibold text-gray-900">{{ $item->producto->categoria->nombre ?? 'N/A' }}:</strong> 
                                {{ $item->producto->nombre ?? 'Producto' }} 
                                <span class="text-gray-500 text-xs font-mono ml-1">(Serie: {{ $item->serie }})</span>
                            </span>
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-bold tracking-wide shadow-sm
                                {{ $item->estado === 'instalado' ? 'bg-emerald-600 text-white' : 'bg-blue-600 text-white' }}">
                                {{ ucfirst($item->estado) }}
                            </span>
                        </div>
                    @empty
