...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <div class="d-flex flex-column flex-lg-row align-items-center gap-2 ms-lg-3 mt-3 mt-lg-0">
                    <a href="{{ $whatsappLink }}" target="_blank" class="btn btn-glow rounded-pill px-3 py-2 fw-bold text-nowrap w-100 w-lg-auto">
                ...
                <div class="row align-items-center gy-5">
                    <div class="col-lg-7 text-lg-start text-center" data-aos="fade-right" data-aos-duration="1000">
                        ...
                        <h1 class="hero-heading">
                            {{ $sections['hero']->title ?? 'Potencia tu Vehículo con' }} <br>
                            <span class="gradient-text">{{ $sections['hero']->subtitle ?? 'GNV & GLP de 5ta Gen' }}</span>
                        </h1>
                        <p class="hero-lead">
                            {{ $sections['hero']->description ?? 'Conversiones de alta precisión, certificaciones oficiales y mantenimiento especializado. Ahorra hasta un 60% en combustible con total seguridad.' }}
                        </p>
                        <div class="d-flex flex-column flex-sm-row gap-3 justify-content-lg-start justify-content-center mt-4">
                            <a href="{{ $whatsappLink }}" target="_blank" class="btn btn-main btn-lg rounded-pill">
                ...
                <div class="col-lg-6" data-aos="fade-left" data-aos-duration="1000">
                    <div class="ps-lg-4">
                        ...
                        <h2 class="section-title fw-bold mb-4">
                            {{ $sections['about']->title ?? 'Líderes en Conversiones y' }} <br>
                            <span class="gradient-text">{{ $sections['about']->subtitle ?? 'Soluciones Automotrices' }}</span>
                        </h2>
                        <p class="text-sub mb-4">
                            {{ $sections['about']->description ?? 'En Arturo Motors nos dedicamos a transformar la movilidad de nuestros clientes ofreciendo soluciones de conversión a GNV y GLP con los más altos estándares de calidad y seguridad.' }}
                        </p>
                        <p class="text-sub mb-4">
        ...
        <div class="container py-lg-5">
            <div class="text-center mb-5" data-aos="fade-up" data-aos-duration="1000">
                ...
                <h2 class="section-title fw-bold">
                    {{ $sections['services']->title ?? 'Nuestros Servicios' }} <span class="gradient-text">{{ $sections['services']->subtitle ?? 'Especializados' }}</span>
                </h2>
                <p class="text-sub mx-auto" style="max-width: 600px;">
                    {{ $sections['services']->description ?? 'Tecnología de última generación y personal altamente capacitado para garantizar el máximo rendimiento de tu vehículo.' }}
                </p>
            </div>
                            ...
                            @if($service->features)
                                <ul class="service-list list-unstyled">
                                    @foreach($service->features as $feature)
                                        <li><i class="fa-solid fa-check text-blue me-2"></i>{{ $feature }}</li>
                                    @endforeach
                                </ul>
                            @endif
                            <a href="{{ $service->cta_link }}" target="_blank" class="btn btn-service w-100 mt-auto">
                                {{ $service->cta_text }} <i class="fa-solid fa-arrow-right ms-2"></i>
                            </a>
                        </div>
                    </div>
                @empty
        ...
        <div class="container py-lg-5">
            ...
            <div class="row align-items-center mb-6 gy-5">
                <div class="col-lg-5" data-aos="fade-right" data-aos-duration="1000">
                    ...
                    <h2 class="section-title fw-bold mb-4">
                        {{ $sections['why']->title ?? '¿Por qué confiar tu vehículo en' }} <br>
                        <span class="gradient-text">{{ $sections['why']->subtitle ?? 'Arturo Motors?' }}</span>
                    </h2>
                    <p class="text-sub mb-4">
                        {{ $sections['why']->description ?? 'Nos enfocamos en brindar un servicio transparente, rápido y respaldado por normativas vigentes para que conduzcas con total tranquilidad y máximo ahorro.' }}
                    </p>
                    <a href="{{ $whatsappLink }}" target="_blank" class="btn btn-main rounded-pill px-4 py-3 fw-semibold">
                        ...
                        @forelse($whyCards as $card)
                            <div class="col-sm-6">
                                <div class="why-card">
                                    ...
                                    <h5 class="fw-bold text-white mb-2">{{ $card->title }}</h5>
                                    <p class="small text-sub mb-0">{{ $card->description }}</p>
                                </div>
                            </div>
                        @empty
            ...
            <div class="mt-4 pt-5 border-top border-secondary-subtle">
                <div class="text-center mb-5" data-aos="fade-up">
                    ...
                    <h2 class="section-title fw-bold">
                        {{ $sections['process']->title ?? 'Nuestro Proceso de' }} <span class="gradient-text">{{ $sections['process']->subtitle ?? 'Trabajo' }}</span>
                    </h2>
                    <p class="text-sub mx-auto" style="max-width: 550px;">
                        {{ $sections['process']->description ?? 'Un flujo transparente y seguro desde la recepción hasta la entrega de tu auto.' }}
                    </p>
                </div>
                    ...
                    @forelse($steps as $index => $step)
                        <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="{{ ($index + 1) * 100 }}">
                            <div class="step-card">
                                ...
                                <h5 class="fw-bold text-white mb-2">{{ $step->title }}</h5>
                                <p class="small text-sub mb-0">{{ $step->description }}</p>
                            </div>
                        </div>
                    @empty
                        <div class="col-12 text-center py-3">
    ...
    <section id="contacto" class="contact-section py-6">
        <div class="container py-lg-5">
            <div class="text-center mb-5" data-aos="fade-up">
                ...
                <h2 class="section-title fw-bold">
                    {{ $sections['contact']->title ?? 'Visítanos o' }} <span class="gradient-text">{{ $sections['contact']->subtitle ?? 'Ponte en Contacto' }}</span>
                </h2>
                <p class="text-sub mx-auto" style="max-width: 550px;">
                    {{ $sections['contact']->description ?? 'Estamos listos para atender tus consultas, certificar tu auto o agendar tu conversión.' }}
                </p>
            </div>
            <div class="row g-4 align-items-stretch">
                    ...
                    <div class="d-flex flex-column gap-3 h-100 justify-content-between">
                        @if(isset($contacts['address']))
                            <div class="contact-info-card">
                                <div class="contact-icon"><i class="{{ $contacts['address']['icon'] }}"></i></div>
                                <div>
                                    <h6 class="fw-bold text-white mb-1">{{ $contacts['address']['label'] }}</h6>
                                    <p class="small text-sub mb-0">{{ $contacts['address']['value'] }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if(isset($contacts['phone']))
                            <div class="contact-info-card">
                                <div class="contact-icon"><i class="{{ $contacts['phone']['icon'] }}"></i></div>
                                <div>
                                    <h6 class="fw-bold text-white mb-1">{{ $contacts['phone']['label'] }}</h6>
                                    <p class="small text-sub mb-0">{{ $contacts['phone']['value'] }}</p>
                                </div>
                            </div>
                        @endif

                        @if(isset($contacts['schedule']))
                            <div class="contact-info-card">
                                <div class="contact-icon"><i class="{{ $contacts['schedule']['icon'] }}"></i></div>
                                <div>
                                    <h6 class="fw-bold text-white mb-1">{{ $contacts['schedule']['label'] }}</h6>
                                    <p class="small text-sub mb-0">{{ $contacts['schedule']['value'] }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="whatsapp-cta-box text-center p-4 rounded-4 mt-2">
                            ...
                            <a href="{{ $whatsappLink }}" target="_blank" class="btn btn-success rounded-pill w-100 fw-bold py-2">
                ...
                <div class="col-lg-7" data-aos="fade-left" data-aos-duration="1000">
                    <div class="map-wrapper h-100 rounded-4 overflow-hidden border border-secondary-subtle">
                        @if(isset($contacts['map_iframe']))
                            {!! $contacts['map_iframe']['value'] !!}
                        @else
                            <iframe 
        ...
        <div class="container">
            <div class="row gy-4 align-items-center">
                ...
                <div class="col-md-4 text-center text-md-end">
                    ...
                    <p class="small text-sub mb-0">&copy; {{ date('Y') }} Arturo Motors. Todos los derechos reservados.</p>
                </div>
            </div>
        </div>
    </footer>
    ...
    <a href="{{ $whatsappLink }}" target="_blank" class="whatsapp-btn-pulse" aria-label="Contacto WhatsApp">
